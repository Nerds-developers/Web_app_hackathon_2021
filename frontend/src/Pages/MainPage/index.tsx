...
import {AppBar, Box, Divider, Grid, Tab, Tabs, Theme} from "@material-ui/core";
...
const MainPage = ({ apiClient, filterConfigs }: MainPageProps) => {
    ...
    const handleChange = (event: React.ChangeEvent<{}>, newValue: 'all'| Shop) => {
        setTab(newValue);
    };
    useEffect(() => {
    ...
    return (
        <Container maxWidth="lg" className={classes.root}>
            ...
            <Grid container direction="row" justify="flex-start" alignItems="flex-start" className={classes.body}>
                ...
                <Grid item xs={8} md={8}>
                    <ProductList data={tab === 'all' ? data : filterByPatter(data, { shop: tab }) }/>
                </Grid>
            </Grid>
        </Container>
    );
